...
{
    public class PointEdgeFar0Calc : PointBase
    {
        ...
        public override List<Point3D> Calc(bool isHorizontal)
        {
            ...
            Point3D controlPoint = Add(center12, Bi2Middle.ToPoint3D());

            for (int i = 0; i < 10; i++)
            {
                Point3D a = controlPoint;
                Point3D b = PointThroughMiddle;
                Point3D c = PointThroughFar0;
                double r;
                double A;
                double B;
                double C;
                if (isHorizontal)
                {
                    r = Mul(PointThroughFar0, PointThroughMiddle, (double)i / 10).X;
                    A = c.X + b.X - 2 * a.X;
                    B = 2 * (a.X - b.X);
                    C = b.X - r;
                }
                ...
                {
                    r = Mul(PointThroughFar0, PointThroughMiddle, (double)i / 10).Y;
                    A = c.Y + b.Y - 2 * a.Y;
                    B = 2 * (a.Y - b.Y);
                    C = b.Y - r;
                }
                ...
                Point3D P10 = Mul(controlPoint, PointThroughMiddle, xd);
                Point3D P11 = Mul(PointThroughFar0, controlPoint, xd);
