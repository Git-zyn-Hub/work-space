...
namespace SurfacePlot
{
    public class SurfacePlotVisual3D : ModelVisual3D
    {
        ...
        private Model3D CreateModel()
        {
            ...
            {
                double j = maxX == minX ? 0 : (x - minX) / (maxX - minX) * (rows - 1);
                var path = new List<Point3D> { new Point3D(x, minY, minZ) };
                for (int i = 0; i < columns; i++)
                {
                    path.Add(maxX == minX ? Points[(int)j, i] : BilinearInterpolation(Points, j, i));
                }
                path.Add(new Point3D(x, maxY, minZ));

                axesMeshBuilder.AddTube(path, LineThickness, 9, false);
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D((x / 10).ToString(), Brushes.Black, true, FontSize,
            ...
            {
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D("T-轴", Brushes.Black, true, FontSize,
            ...
            for (double y = minY; y <= maxY; y += IntervalY)
            {
                ...
                double i = maxY == minY ? 0 : (y - minY) / (maxY - minY) * (columns - 1);
                var path = new List<Point3D> { new Point3D(minX, y, minZ) };
                for (int j = 0; j < rows; j++)
                {
                    path.Add(rows == 1 ? Points[j, (int)i] : BilinearInterpolation(Points, j, i));
                }
                ...
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D(yAxisLabel, Brushes.Black, true, FontSize,
                                                                           new Point3D(minX - FontSize * 3, y, minZ),
                                                                           new Vector3D(1, 0, 0), new Vector3D(0, 1, 0));
                plotModel.Children.Add(label);
            }
            {
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D("F-轴", Brushes.Black, true, FontSize,
            ...
            {
                GeometryModel3D label = TextCreator.CreateTextLabelModel3D("A-轴", Brushes.Black, true, FontSize,
